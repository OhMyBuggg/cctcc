//create categories
LOAD CSV WITH HEADERS FROM 'file:///test1.csv' AS list
//WITH trim(list.Type) AS type, trim(list.Categories) AS categories, list.Title AS title WHERE type='Category'
WITH trim(list.Type) AS type, trim(list.Categories) AS categories, list.Title AS title, list.Url AS url WHERE type='Category'
//CREATE (n:Category{title:trim(title),parents:categories}) RETURN *;
...
LOAD CSV WITH HEADERS FROM 'file:///test1.csv' AS list
//WITH trim(list.Type) AS type, trim(list.Categories) AS categories, list.Title AS title WHERE type='Individual'
